...
int isLess(int x, int y) {
    int result = x + (~y + 1); 
   int check1 = x & (~y); 
   int check2 = (~(x^y)) & result; 

   return (check1 | check2) >> 31 & 1;  
}
/* 
...
unsigned float_i2f(int x) {
   ...
   if(x == 0){
      ...
      if(sign){ // sign != 0 means x is negative 
         x = ~x + 1; 
      }
      bit = 1; 
      while((x >> bit) != 0){
...
int float_f2i(unsigned uf) {
   ...
   unsigned uf_exp = (uf & 0x7F800000) >> 23;
   unsigned uf_frac = (uf & 0x007FFFFF); 
   unsigned bias = 0x7F; 
   unsigned res = uf_frac; 
   ...
   if(uf_exp == 0xFF){
      ...
      if(uf_exp >= 31){
         ...
         res = uf_frac << (uf_exp - 23); 
      }
      else{
         res = uf_frac >> (23 - uf_exp); 
      }
